...
| 12 | **Human Oversight** *(v1.7, extended v1.2)* | AI Act Art. 14 review trail — live `GET /human-reviews` with fixture fallback. Two feeds now land here: the IamDelegation bridge (core ≥ 1.8) writes **delegation grants** — what an agent *may* do — and the AI-runtime bridge (core ≥ 1.9, `laravel/ai` ^0.11) writes **per-action tool approvals** — what an agent was *about to* do, which is the evidence an auditor asks for once the action had an effect | Subject-type labels with humanize fallback (**Delegation grant** and **Tool approval** first-class) + state pills + **Outcome** column that says which of the two happened — *denied, the tool did not run* vs *approved, the tool ran* — instead of leaving `rejected` to mean either + **Run** column: one click pivots the whole trail to a single invocation + drawer with the call behind the decision (agent · tool · tool-call id · run · conversation), the model's stated reason marked as a claim, and the full review notes. **v1.3** adds the third feed — the Routines bridge (core ≥ 1.10) writes the **standing mandate** a human granted an unattended routine and every **pause-and-ask** it produced; a paused run is labelled *Routine pause*, not *Routine run*, because the record is a question waiting for an answer, and the header counts **how many routines are waiting** — that pause is the one oversight item that rots invisibly, since the routine is behaving exactly as designed and therefore reports nothing anywhere |
