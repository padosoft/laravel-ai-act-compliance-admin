...
function PageOverview({ onNavigate, onOpenIncident }) {
  const lastRefreshed = window.NOW;

  // ---- compute KPIs from data ----
  const dsarOpen   = DSAR.filter(d => d.status !== 'completed' && d.status !== 'rejected');
  ...
  const consentAvg = (
    Object.values(CONSENT_RATE).reduce((a, b) => a + b.granted, 0) /
    Object.keys(CONSENT_RATE).length
  );

  // bias: worst language cohort
  const worstCohort = COHORT_DATA.language.rows.reduce((acc, r) =>
    r.accuracy < acc.accuracy ? r : acc, COHORT_DATA.language.rows[0]);
  ...
  const kpis = [
    {
      ...
      sub: <span>{dsarBreached > 0 ? <><b style={{color:'var(--sev-critical)'}}>{dsarBreached}</b> breached</> : <>0 breached</>} · <span className="muted">{dsarSoon} due ≤ 5d</span></span>,
    ...
    },
    {
      ...
      sub: (
        <div className="kpi-mini-bars">
          <Tooltip label={`${critical} critical`}><div className="mb" style={{background:'var(--sev-critical)', height: critical * 4 + 4}}/></Tooltip>
          <Tooltip label={`${high} high`}><div className="mb" style={{background:'var(--sev-high)', height: high * 4 + 4}}/></Tooltip>
          <Tooltip label={`${medium} medium`}><div className="mb" style={{background:'var(--sev-medium)', height: medium * 4 + 4}}/></Tooltip>
          <Tooltip label={`${low} low`}><div className="mb" style={{background:'var(--sev-low)', height: low * 4 + 4}}/></Tooltip>
        </div>
      ),
  ...
  const alertBanner = critical > 0
    ? { kind: 'critical', text: `${critical} critical incident${critical > 1 ? 's' : ''} require immediate attention`, route: 'incidents' }
    : dsarBreached > 0
      ? { kind: 'warn', text: `${dsarBreached} DSAR breached GDPR Art. 15 / 17 30-day SLA`, route: 'dsar' }
      : null;
    ...
    <div className="page" data-screen-label="Overview">
      <div className="page-head">
        ...
        <div className="page-actions">
          <button className="btn"><I.Download size={13}/> Export snapshot</button>
          <button className="btn primary" onClick={() => onNavigate('dpo')}>
      ...
      {alertBanner && (
        <div className={`alert-banner ${alertBanner.kind === 'warn' ? 'warn' : ''}`}>
          <I.AlertTriangle size={16} className="icon"/>
          <div className="grow">
            <b>{alertBanner.kind === 'critical' ? 'Critical alert' : 'SLA warning'}</b> · {alertBanner.text}
          </div>
          <button className="btn sm" onClick={() => onNavigate(alertBanner.route)}>
      ...
      <div className="kpi-grid">
        {kpis.map(k => (
          <div key={k.key}
               className={`kpi clickable ${k.tone === 'alert' ? 'alert' : k.tone === 'warn' ? 'warn' : ''}`}
               onClick={k.onClick}>
            ...
            <div style={{marginTop:6, fontSize:11.5, color:'var(--text-secondary)'}}>{k.sub}</div>
            {k.spark && (
              <div className="kpi-spark">
                <Sparkline data={k.spark} color={k.tone === 'alert' ? 'var(--sev-critical)' : k.tone === 'warn' ? 'var(--sev-medium)' : 'var(--accent)'}/>
              </div>
            )}
          </div>
        ))}
      ...
      <div className="grid-2-1-2">
        <div className="card">
          <div className="card-head">
            ...
            <button className="btn sm ghost">View all <I.ChevronRight size={11}/></button>
          </div>
          <div className="card-body flush">
            <div className="feed">
              {ACTIVITY.slice(0, 10).map((a, i) => (
                <div className="feed-row" key={i} onClick={() => {
                  if (a.kind.startsWith('incident')) onNavigate('incidents');
                  else if (a.kind.startsWith('dsar')) onNavigate('dsar');
                  else if (a.kind.startsWith('consent')) onNavigate('consent');
                  else if (a.kind.startsWith('risk')) onNavigate('risks');
                  else if (a.kind.startsWith('bias')) onNavigate('bias');
                  else if (a.kind === 'attestation' || a.kind === 'retention') onNavigate('dpo');
                  else if (a.kind === 'webhook') onNavigate('settings');
                }}>
                  <span className={`feed-icon ${a.severity}`}>{FEED_ICONS[a.kind] || <I.Activity size={12}/>}</span>
                  <div className="feed-body">
                    <b>{a.text}</b>
                    <small>by {a.actor}</small>
                  </div>
                  <time className="feed-time">{fmtRelativeFrom(a.at)}</time>
                </div>
              ))}
            </div>
          </div>
      ...
      <div className="card mt-16">
        <div className="attest-card">
          ...
          <div>
            ...
            <p>
              Last DPO review: <b style={{color:'var(--text)'}}>{fmtDateLong(window.NOW - 23 * 86400_000)}</b> ·
              {' '}<span className="badge success" style={{marginLeft:4}}><span className="dot"/>Current</span>
...
// --- DSAR queue depth line chart (right column) ---
function DsarDepthChart({ data }) {
  const [hover, setHover] = React.useState(null);
  const W = 480, H = 220, PAD = { t: 20, r: 20, b: 28, l: 32 };
  const innerW = W - PAD.l - PAD.r;
  const innerH = H - PAD.t - PAD.b;
  const max = Math.max(...data, 8);
  const sla = 8; // visual threshold
    ...
    <div className="card">
      ...
      <div className="card-body" style={{paddingTop:8}}>
        <svg viewBox={`0 0 ${W} ${H}`} style={{width:'100%', height:240}}
          ...
          {/* SLA band */}
          <rect x={PAD.l} y={yAt(sla)} width={innerW} height={PAD.t + innerH - yAt(sla)}
                fill="var(--sev-medium)" fillOpacity="0.06"/>
          <line x1={PAD.l} x2={PAD.l + innerW} y1={yAt(sla)} y2={yAt(sla)}
                stroke="var(--sev-medium)" strokeOpacity="0.4" strokeDasharray="3 3"/>
          <text x={PAD.l + innerW - 4} y={yAt(sla) - 4} textAnchor="end"
                style={{fontSize:9, fill:'var(--sev-medium)', fontFamily:'var(--font-mono)'}}>SLA target</text>

          {/* Y gridlines */}
          {[0, max/2, max].map((v, i) => (
            <g key={i}>
              <line x1={PAD.l} x2={PAD.l + innerW} y1={yAt(v)} y2={yAt(v)} stroke="var(--border)" strokeDasharray="2 3"/>
              <text x={PAD.l - 6} y={yAt(v) + 3} textAnchor="end"
                    style={{fontSize:9, fill:'var(--text-tertiary)', fontFamily:'var(--font-mono)'}}>{Math.round(v)}</text>
            </g>
          ))}
          ...
          {[0, 7, 14, 21, 29].map(i => (
            <g key={i}>
              ...
              <text x={xAt(i)} y={PAD.t + innerH + 14} textAnchor="middle"
          ...
          {hover && (
            <g pointerEvents="none">
              <line x1={xAt(hover.i)} x2={xAt(hover.i)} y1={PAD.t} y2={PAD.t + innerH}
                    stroke="var(--text)" strokeOpacity="0.2"/>
              <circle cx={xAt(hover.i)} cy={yAt(hover.v)} r="3.5" fill="var(--accent)" stroke="var(--bg-elevated)" strokeWidth="2"/>
              <g transform={`translate(${xAt(hover.i)}, ${yAt(hover.v) - 14})`}>
